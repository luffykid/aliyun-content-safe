...
import com.kid.aliyuncontentsafe.MediaComparison;
...
public class AliyunContentSafeTest {
    ...
    @Test
    public void testScanLogoVideo() throws InterruptedException {
        AliyunContentSafe aliyunContentSafe = new AliyunContentSafe();
        String videoUri = "http://139.219.13.249/vimg/icon/market/efe127fa-au=3798078917,1929861432&fm=15&gp=0.jpg";
        String result = aliyunContentSafe.asynScanLogoVideo(Arrays.asList(ScanMedia.newInstance(videoUri)));
        Assert.assertEquals("200", JsonPath.read(result, "$.code").toString());
        System.out.println(result);
        Assert.assertEquals("1", JsonPath.read(result, "$.data.length()").toString());
        Thread.sleep(30000);
    ...
    @Test
    public void testSfaceFaceImageComparison() {
        AliyunContentSafe aliyunContentSafe = new AliyunContentSafe();
        String imageUrl = "http://139.219.13.249/vimg/icon/market/1.png";
        String result = aliyunContentSafe
                            .sfaceFaceImageComparison(
                                                Arrays.asList(MediaComparison.newInstance(
                                                        imageUrl,
                                                        imageUrl
                                                )));
        System.out.println(result);
        ...
        Assert.assertEquals("200", JsonPath.read(result, "$.code").toString());
        Assert.assertEquals("sface-1", JsonPath.read(result,"$.data[0].results[0].label"));
